...
describe('Auth Service', () => {
  ...
  it('should register a new user Paul Atreides', async () => {
    query.mockResolvedValueOnce({ rows: [{ id: 1, email: 'paul.atreides@arrakis.com' }] });
  ...
  it('should register a new user Lisan Al Gaib', async () => {
    query.mockResolvedValueOnce({ rows: [{ id: 2, email: 'lisan.al.gaib@arrakis.com' }] });
  ...
  it('should login a user Paul Atreides', async () => {
    ...
    query.mockResolvedValueOnce({ rows: [{ id: 1, email: 'paul.atreides@arrakis.com', password: hashedPassword }] });
  ...
  it('should login a user Lisan Al Gaib', async () => {
    ...
    query.mockResolvedValueOnce({ rows: [{ id: 2, email: 'lisan.al.gaib@arrakis.com', password: hashedPassword }] });
